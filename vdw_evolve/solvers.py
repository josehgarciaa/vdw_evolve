...
from .solvers_utils import annealing_sc, mechanic_sc
...
class AnnealingSolver:
    ...
    def solve(self, cel1, cel2):
        # cel1 = np.dot(cel)
        ta, strain_tb, t_cel2_no_strain, diagonal_strain, strain = annealing_sc(cel1, cel2, self.nr_epochs, self.model_par, self.fit_f)
...
class MechanicSolver:
    ...
    def solve(self, cel1, cel2):
        ta, strain_tb, diagonal_strain, strain = mechanic_sc(cel1, cel2, exploring_range=self.exploring_range, tolerance=self.tolerance, paralel_limit=self.paralel_limit )
        sc = SuperCell(parents=(cel1, cel2), transformation=(ta, strain_tb), strains=(strain, diagonal_strain))
        return sc
