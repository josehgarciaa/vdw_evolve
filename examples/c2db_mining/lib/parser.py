...
def get_data_as_pd(options, props, path="./c2db.db"):
    """
    Example:
...
def extract_structure(uid_list, save_path="structures/"):
    for uid in uid_list:
        ...
        if os.path.isfile(file):
            ...
            time.sleep(2)
            try:
                current_struct= wget.download(data_url, out="./" + save_path)
                print(current_struct)
                os.rename(dw_path, file)
            except BaseException as e:
                print('Failed to do download: ' + str(e))
